...
describe('UsersService', () => {
  ...
  beforeEach(() => {
    ...
    localStorageSpy = jasmine.createSpyObj('LocalStorageService', ['removeItem', 'getItem', 'setItem'],
      {
        names: {
          user: "user",
        }
  ...
  it('UsersService can not create a user', (done) => {
    httpClientSpy.post.and.returnValue(throwError(() =>
      new HttpErrorResponse({status: 500, statusText: 'Can not create a user'}))
      .pipe(delay(1000)));
  ...
  it('UsersService can update currency', (done) => {
    ...
    const add$ = userService.updateCurrency(newCurrency, {returnObs: true}) as Observable<User>;
    const reset$ = userService.updateCurrency(newCurrency, {returnObs: true, hardSet: true}) as Observable<User>
  ...
  it('UsersService can update online', (done) => {
    ...
    const onOnline600$ = userService.updateOnline({...newOnline, claimed: 23}, true) as Observable<User>;
  ...
  it('UsersService calls user exist', fakeAsync(() => {
    ...
    function getResponse(url: string) {
      ...
      return response.status === 500 ? new HttpErrorResponse({status: 500, statusText: 'Error 500'}) : response;
    }
