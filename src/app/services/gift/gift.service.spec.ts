...
describe('GiftService', () => {
  ...
  const giftConfig: GiftConfig = {lastLogin: "12/13/2024", userId: fakeUser.id};

  beforeEach(() => {
    httpClientSpy = jasmine.createSpyObj('HttpClient', ['get', 'post', 'put']);

    httpClientSpy.get.and.returnValue(of([giftConfig]));
    httpClientSpy.post.and.callFake((url: string, body: Partial<User>, options?: any) => {
  ...
  it('GiftService should return config', (done) => {
    ...
    giftService.getConfig(callbackSpy)
      .subscribe({
        next: (response) => {
