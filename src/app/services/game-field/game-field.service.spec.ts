...
describe('GameFieldService', () => {
  ...
  beforeEach(() => {
    gameActionServiceSpy = jasmine.createSpyObj('GameService', ['getFixedDefence', 'getFixedAttack']);
  ...
  it('GameFieldService should return damage', () => {
    const units = {dmgTaker: heroesService.getBrownWolf(), attackDealer: heroesService.getGiant()};
  ...
  it('GameFieldService should determine af a position can be reached', () => {
    ...
    //Should be not ok
    canReachPosition = gameFieldService.canReachPosition(grid, start, {...end, i: 4});
