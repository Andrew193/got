...
describe('UnitService', () => {
  ...
  beforeEach(() => {
    ...
    units = [{...heroService.getGiant(), x: 1, y: 1}, {
  ...
  it('UnitService should order units by distance', () => {
    const Points: Coordinate[] = [{x: 1, y: 3}, {x: 5, y: 3}, {x: 3, y: 3}];

    const orderedPoints = unitService.orderUnitsByDistance({x: 0, y: 0}, Points);

    expect(orderedPoints).toEqual([{x: 1, y: 3}, {x: 3, y: 3}, {x: 5, y: 3}]);
  })
  ...
  it('UnitService should add debuffs to unit', () => {
    ...
    const getEffectsWithIgnoreFilterSpy = jasmine.createSpy('getEffectsWithIgnoreFilter').and.callFake(effectsService.getEffectsWithIgnoreFilter)

    const updatedUnit = unitService.addEffectToUnit(units, 0, testSkill, false, getEffectsWithIgnoreFilterSpy);
  ...
  it('UnitService should add buffs to unit', () => {
    ...
    expect(buffsNames).toEqual([effectsService.effects.healthRestore, effectsService.effects.defBuff]);
  })
  ...
  it('UnitService should update units on grid', () => {
    ...
    grid = grid.map((innerArray, index) => {
      return innerArray.map((tile, position) => ({...tile, x: index, y: position}))
    })
