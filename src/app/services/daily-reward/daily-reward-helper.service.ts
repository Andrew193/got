...
export class DailyRewardHelperService {
  ...
  getWeekReward = (weekIndex0: number, totalDays: number, itemsInSlice?: number): DayReward[] => {
    ...
    return this.baseWeek(1)
      .slice(0, itemsInSlice || totalDays)
      .map(r => ({
  ...
  monthReward = (totalDays: number, itemsInSlice?: number): DayReward[] => {
    const rewards: DayReward[] = [];
    const total = totalDays || 1;

    this.daysCoins.clear();

    for (let week = 0; week < 4; week++) {
      rewards.push(...this.getWeekReward(week, total, itemsInSlice));
    }
