    ...
    attackUser(intervalFight = true, aiMove = true) {
        ...
        const makeAiMove = (aiUnit: Unit, index: number) => {
            ...
            if (aiUnit.health && aiUnit.canMove) {
                ...
                const closestUserUnits = this.fieldService.orderUnitsByDistance(aiUnit, this[aiMove ? 'userUnits' : 'aiUnits']);
                ...
                for (let i = 0; i < closestUserUnits.length; i++) {
                    ...
                    if (userUnit.health) {
                        ...
                        //Move AI unit
                        this[aiMove ? 'aiUnits' : 'userUnits'][index] = {...this[aiMove ? 'aiUnits' : 'userUnits'][index], canMove: false, x: canGetToUnit.i, y: canGetToUnit.j}
                        ...
                        if (enemyWhenCannotMove) {
                            ...
                            const userIndex = this.fieldService.findUnitIndex(this[aiMove ? 'userUnits' : 'aiUnits'], this.fieldService.getUnitFromPosition(enemyWhenCannotMove));
                            ...
                            this[aiMove ? 'aiUnits' : 'userUnits'][index] = {...this[aiMove ? 'aiUnits' : 'userUnits'][index], canAttack: false, skills: skills};
                            this.gameConfig = this.fieldService.getGameField(this[aiMove ? 'userUnits' : 'aiUnits'], this[aiMove ? 'aiUnits' : 'userUnits'], this.fieldService.getDefaultGameField());
                            return;
                        } else {
                            //Dead AI units do not make moves
                            this[aiMove ? 'aiUnits' : 'userUnits'][index] = {...this[aiMove ? 'aiUnits' : 'userUnits'][index], canAttack: false};
                        }
                    }
                }
            }
