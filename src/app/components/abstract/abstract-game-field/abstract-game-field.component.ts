...
export abstract class AbstractGameFieldComponent extends GameFieldVars {
    ...
    logEvent(props: { damage: number | null, newHealth: number | null, addDmg?: number }, isUser: boolean, skill: Skill | Effect, dmgTaker: Unit, message?: string) {
        ...
        if (message) {
            ...
            if (props.newHealth === 0) {
                logMsg(message || `${!isUser ? 'Player' : 'Bot'} ${dmgTaker.name} (${dmgTaker.x + 1})(${dmgTaker.y + 1}) to the seven!`)
            }
        }
    }
}
