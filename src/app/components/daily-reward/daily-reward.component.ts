...
export class DailyRewardComponent {
  ...
  getWeekReward(copperCoinM = 1, silverCoin = 1, vipPointsM = 1, summonCardM = 1, summonScrollM = 1, dayM = 1): DayReward[] {
    return [
      ...
      {copperCoin: +(25000 * (copperCoinM + 0.2 * dayM)).toPrecision(3), silverCoin: +(1000 * silverCoin * dayM).toPrecision(3)},
      {copperCoin: +(35000 * (copperCoinM + 0.7 * dayM)).toPrecision(3), goldCoin: +(250 * (silverCoin + 0.3 * dayM)).toPrecision(3)},
      {copperCoin: +(45000 * (copperCoinM + 1.2 * dayM)).toPrecision(3), summonCard: +(1 * summonCardM * dayM).toPrecision(3)},
      {copperCoin: +(55000 * (copperCoinM + 1.4 * dayM)).toPrecision(3), summonScroll: +(1 * summonScrollM * dayM).toPrecision(3)},
